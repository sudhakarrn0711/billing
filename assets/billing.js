...
const API_BASE = "https://script.google.com/macros/s/AKfycbykzrFOqAVpCb1Glsz_Qa5FXkwJSEWFXbzOuPJjBuQEWO14PdacgUvJ4EsaN_zR-_DC/exec"; // <-- Replace with deployed URL
...
<td class="p-2 flex items-center gap-2">
  ...
  <!-- Edit -->
  <button 
    class="p-1.5 rounded-lg text-blue-400 ${isViewer ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-600/20'}"
    ${isViewer ? 'disabled' : `onclick="openEditInvoice('${inv.id}')"`} 
    title="Edit Invoice">
    <i data-lucide="pencil"></i>
  </button>

  <!-- Payment -->
  <button class="p-1.5 rounded-lg hover:bg-emerald-600/20 text-emerald-400" 
    onclick="recordPayment('${inv.id}')" title="Record Payment">
    <i data-lucide="credit-card"></i>
  </button>

  <!-- Print -->
  <button class="p-1.5 rounded-lg hover:bg-amber-600/20 text-amber-400" 
    onclick="openPrint('${inv.id}')" title="Print Invoice">
    <i data-lucide="printer"></i>
  </button>

  <!-- Bulk Payment -->
  <button class="p-1.5 rounded-lg hover:bg-indigo-600/20 text-indigo-400" 
    onclick="openBulkPaymentModal('${cust.id}')" title="Record Bulk Payment">
  ...
  <!-- Delete -->
  <button 
    class="p-1.5 rounded-lg text-red-400 ${isViewer ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-600/20'}"
    ${isViewer ? 'disabled' : `onclick="openDeleteInvoiceModal('${inv.id}', '${invoiceLabel}')"`} 
...
async function sendWhatsAppReminder(invoiceId) {
  try {
    ...
    if (!customer || !customer.phone) throw new Error("Customer not found");
    ...
    if (!pendingInvoices.length) {
      showToast(`✅ No pending invoices`, "info");
      return;
    }
    ...
    const invoiceNumbers = pendingInvoices.map(inv => {
      const due = new Date(inv.dueDate || inv.date || today);
    ...
    const totalAmount = pendingInvoices.reduce((s, i) => s + Number(i.total || 0), 0);
    const totalPaid = pendingInvoices.reduce((s, i) =>
      s + (i.payments || []).reduce((x, p) => x + Number(p.amount || 0), 0), 0);

    const totalPending = totalAmount - totalPaid;

    const latestDueDate = new Date(
      Math.max(...pendingInvoices.map(i => new Date(i.dueDate || i.date || 0)))
    );

    // ✅ PAYMENT DETAILS
    const upiId = "f051m00855@indianbk";
    const gpay = "9363410567";
    const businessName = "Ranjan Travels";

    const upiWebLink = `https://pay.google.com/gp/p/ui/pay?pa=${upiId}&pn=${encodeURIComponent(businessName)}&am=${totalPending}&cu=INR`;

    const dashboardLink = `https://billing.ransangroups.in/dashboard.html?customerId=${customer.id}`;
...
🧾 Pending Invoices: ${invoiceNumbers.join(", ")}
...
⚠️ Kindly clear before: ${latestDueDate.toISOString().slice(0, 10)}
...
async function saveInvoice() {
  ...
  try {
    ...
    // ✅ AUTO SEND WHATSAPP AFTER SAVE
    try {
      await fetch(API_URL, {
        method: "POST",
        body: JSON.stringify({
          action: "sendInvoiceWhatsApp", // 👈 new action
          invoiceId: savedInv.id,
          sheetId: RUN_ENV   // or pass actual sheetId if needed
        })
      });
...
async function recordPayment(invId) {
  ...
  try {
    ...
    sendPaymentWhatsApp(inv, paidAmount);
  } catch (err) {
...
async function sendPaymentWhatsApp(inv, paidAmount) {
  try {
    const customer = state.customers.find(c => c.id === inv.customerId);
    if (!customer || !customer.phone) return;

    const totalPaid = (inv.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
    const pending = (inv.total || 0) - totalPaid;

    await fetchServerData("sendpaymentwhatsapp", {
    ...
    console.error("WhatsApp Payment Send Error:", err);
  }
}
...
function openPrint(input) {
  ...
  // ✅ CASE 1: Called from MAIN APP (invId)
  if (typeof input === "string") {

    let invData = state.invoices.find(x => x.id === input);
    if (!invData) return;

    inv = Object.assign({}, invData, invData.snapshot || {});
  }

  // ✅ CASE 2: Called from DASHBOARD (full object)
